...
Blockly.Mbed.math_change = function() {
  // Add to a variable in place.
  var argument0 = Blockly.Mbed.valueToCode(this, 'DELTA',
...
Blockly.Mbed.math_single = function() {
  ...
  if (operator == 'NEG') {
    ...
    arg = Blockly.Mbed.valueToCode(this, 'NUM',
...
Blockly.Mbed.math_on_list = function() {
  // Math functions for lists.
  var func = this.getTitleValue('OP');
  var list = Blockly.Mbed.valueToCode(this, 'LIST',
      Blockly.Mbed.ORDER_NONE) || '[]';
  var code;
  switch (func) {
    case 'SUM':
      if (!Blockly.Mbed.definitions_['math_sum']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_sum', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_sum = functionName;
        var func = [];
        ...
        func.push('}');
        Blockly.Mbed.definitions_['math_sum'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_sum + '(' + list + ')';
      break;
    case 'MIN':
      if (!Blockly.Mbed.definitions_['math_min']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_min', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_min = functionName;
        var func = [];
        func.push('num ' + functionName + '(List myList) {');
        ...
        func.push('}');
        Blockly.Mbed.definitions_['math_min'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_min + '(' + list + ')';
      break;
    case 'MAX':
      if (!Blockly.Mbed.definitions_['math_max']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_max', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_max = functionName;
        var func = [];
        func.push('num ' + functionName + '(List myList) {');
        func.push('  if (myList.isEmpty()) return null;');
        ...
        func.push('}');
        Blockly.Mbed.definitions_['math_max'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_max + '(' + list + ')';
      break;
    case 'AVERAGE':
      // This operation exclude null and values that are not int or float:
      //   math_mean([null,null,"aString",1,9]) == 5.0.
      if (!Blockly.Mbed.definitions_['math_average']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_average', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_average = functionName;
        var func = [];
        func.push('num ' + functionName + '(List myList) {');
        ...
        func.push('}');
        Blockly.Mbed.definitions_['math_average'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_average + '(' + list + ')';
      break;
    case 'MEDIAN':
      if (!Blockly.Mbed.definitions_['math_median']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_median', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_median = functionName;
        var func = [];
        func.push('num ' + functionName + '(List myList) {');
        ...
        func.push('  }');
        func.push('}');
        Blockly.Mbed.definitions_['math_median'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_median + '(' + list + ')';
      break;
    case 'MODE':
      if (!Blockly.Mbed.definitions_['math_modes']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_modes', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_modes = functionName;
        ...
        func.push('}');
        Blockly.Mbed.definitions_['math_modes'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_modes + '(' + list + ')';
      break;
    case 'STD_DEV':
      if (!Blockly.Mbed.definitions_['math_standard_deviation']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_standard_deviation', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_standard_deviation = functionName;
        var func = [];
        ...
        func.push('}');
        Blockly.Mbed.definitions_['math_standard_deviation'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_standard_deviation +
          '(' + list + ')';
      break;
    case 'RANDOM':
      if (!Blockly.Mbed.definitions_['math_random_item']) {
        var functionName = Blockly.Mbed.variableDB_.getDistinctName(
            'math_random_item', Blockly.Generator.NAME_TYPE);
        Blockly.Mbed.math_on_list.math_random_item = functionName;
        var func = [];
        func.push('Dynamic ' + functionName + '(List myList) {');
        func.push('  int x = (Math.random() * myList.length).floor().toInt();');
        func.push('  return myList[x];');
        func.push('}');
        Blockly.Mbed.definitions_['math_random_item'] = func.join('\n');
      }
      code = Blockly.Mbed.math_on_list.math_random_item + '(' + list + ')';
      break;
    default:
...
Blockly.Mbed.math_constrain = function() {
  // Constrain a number between two limits.
  var argument0 = Blockly.Mbed.valueToCode(this, 'VALUE',
      Blockly.Mbed.ORDER_NONE) || '0';
  var argument1 = Blockly.Mbed.valueToCode(this, 'LOW',
      Blockly.Mbed.ORDER_NONE) || '0';
  var argument2 = Blockly.Mbed.valueToCode(this, 'HIGH',
      Blockly.Mbed.ORDER_NONE) || '0';
...
Blockly.Mbed.math_modulo = function() {
  // Remainder computation.
  var argument0 = Blockly.Mbed.valueToCode(this, 'DIVIDEND',
      Blockly.Mbed.ORDER_MULTIPLICATIVE) || '0';
  var argument1 = Blockly.Mbed.valueToCode(this, 'DIVISOR',
      Blockly.Mbed.ORDER_MULTIPLICATIVE) || '0';
  var code = argument0 + ' % ' + argument1;
  return [code, Blockly.Mbed.ORDER_MULTIPLICATIVE];
};

Blockly.Mbed.math_random_int = function() {
  // Random integer between [X] and [Y].
  var argument0 = Blockly.Mbed.valueToCode(this, 'FROM',
      Blockly.Mbed.ORDER_NONE) || '0';
  var argument1 = Blockly.Mbed.valueToCode(this, 'TO',
      Blockly.Mbed.ORDER_NONE) || '0';
  if (!Blockly.Mbed.definitions_['math_random_int']) {
    var functionName = Blockly.Mbed.variableDB_.getDistinctName(
    ...
    func.push('}');
    Blockly.Mbed.definitions_['math_random_int'] = func.join('\n');
  }
  var code = Blockly.Mbed.math_random_int.random_function +
      '(' + argument0 + ', ' + argument1 + ')';
  return [code, Blockly.Mbed.ORDER_UNARY_POSTFIX];
};

Blockly.Mbed.math_random_float = function() {
  // Random fraction between 0 and 1.
  return ['Math.random()', Blockly.Mbed.ORDER_UNARY_POSTFIX];
};
